...
class _MPCTensor(_SyftTensor):
    ...
    def __init__(self,
        ...
        if shares is not None:
            ...
            raise TypeError("cannot initialize MPCTensor with shares and child both == None")
        self.torch_type = torch_type
    ...
    def deser(cls, msg_obj, worker, acquire):
        ...
        if acquire:
            ...
            if '___GeneralizedPointerTensor__' in child_shares.keys():
                ...
                # result = sy._MPCTensor(shares=shares,
                #                        id=msg_obj['id'],
                #                        owner=worker,
                #                        torch_type=msg_obj['torch_type'])
                result = sy._MPCTensor(shares=sy.LongTensor())
            else:
...
class _TorchObject(object):
    ...
    def share(self, *workers):
        ...
        if isinstance(self.child, _PointerTensor):
            ...
            x_mpc = _MPCTensor(x_gp, torch_type='syft.LongTensor').on(self)
